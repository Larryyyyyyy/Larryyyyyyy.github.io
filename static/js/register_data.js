...
app.post('/submit', (req, res) => {
    ...
    const { inputUsername1,inputEmail2,inputPassword2 } = req.body;

    if (!inputPassword2 || !inputEmail2 || !inputUsername1) {
        console.error("Lack of password or email or username"); // 输出错误日志
        return res.status(400).json({ error: '缺少密码或邮箱或用户名' });
    }

    const stmt = db.prepare("INSERT INTO users (username,email,password) VALUES (?, ?, ?)");
    stmt.run(inputUsername1,inputEmail2,inputPassword2, function(err) {
        if (err) {
